...
for ep in range(epochs):
    if cfg.state=='train':
        ...
        for x, y in train_loader:
            ...
            y = y_normalizer.decode(y.reshape(batch_size_, s,  s))
    ...
    with torch.no_grad():
        for x, y, in test_loader:
            ...
            y = y_normalizer.decode(y.reshape(batch_size_, s, s))
            loss = myloss(out.view(batch_size_, -1), y.view(batch_size_, -1))
            test_l2_error += loss.item()
            if ep % 10 == 0:
                norms = torch.norm(y.view(batch_size_, -1), dim=1)
                error = y - out
                relative_error = torch.norm(error.view(batch_size_, -1), dim=1) / norms
    ...
    if cfg.state == 'eval':
        ...
        savemat('predictions/FNO/FNO_hh_median_' + string + '_id' + str(idx_median) + '.mat',
        ...
        savemat('predictions/FNO/FNO_hh_max_' + string + '_id' + str(idx_max) + '.mat',
                {'input': input, 'output': output, 'output_true': output_true})
