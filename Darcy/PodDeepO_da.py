...
# prefix = "dataset/"
data = io.loadmat(prefix + "/Darcy_Triangular_40000")
...
if cfg.state=='train':
    ...
    if (cfg.path_model):
        model_state_dict = torch.load(cfg.path_model, map_location=device).state_dict()
...
for ep in range(epochs):
    ...
    if cfg.state == 'eval':
        ...
        savemat('predictions/PodDeepOnet/PodDeepOnet_da_median_' + string + '_id' + str(idx_median) + '.mat',
        ...
        savemat('predictions/PodDeepOnet/PodDeepOnet_da_max_' + string + '_id' + str(idx_max) + '.mat',
                {'input': input, 'output': output, 'output_true': output_true})
